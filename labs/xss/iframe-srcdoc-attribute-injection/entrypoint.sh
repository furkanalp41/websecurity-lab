...
flag = "FLAG{%s}" % hmac.new(s, b"v1|iframe-srcdoc-attribute-injection", hashlib.sha256).hexdigest()
sess = hmac.new(s, b"admin-session|iframe-srcdoc-attribute-injection", hashlib.sha256).hexdigest()[:32]
print("FLAG=%s" % flag)
...
# Single worker: the note store and admin session live in-process.
