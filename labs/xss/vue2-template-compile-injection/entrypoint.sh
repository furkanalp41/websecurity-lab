...
flag = "FLAG{%s}" % hmac.new(s, b"v1|vue2-template-compile-injection", hashlib.sha256).hexdigest()
sess = hmac.new(s, b"admin-session|vue2-template-compile-injection", hashlib.sha256).hexdigest()[:32]
print("FLAG=%s" % flag)
...
# Single worker: the widget store and admin session live in-process.
