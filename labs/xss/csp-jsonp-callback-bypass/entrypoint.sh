...
# Per-container flag (HMAC over the slug) + the admin session cookie the XSS must
# steal. /solve returns the flag when the exfiltrated cookie matches the session.
python - <<'PY' > /tmp/lab-secrets.env
import hashlib, hmac, os
s = os.environ["LAB_USER_SECRET"].encode()
flag = "FLAG{%s}" % hmac.new(s, b"v1|csp-jsonp-callback-bypass", hashlib.sha256).hexdigest()
sess = hmac.new(s, b"admin-session|csp-jsonp-callback-bypass", hashlib.sha256).hexdigest()[:32]
print("FLAG=%s" % flag)
