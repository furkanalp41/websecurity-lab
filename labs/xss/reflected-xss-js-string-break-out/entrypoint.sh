...
# /solve returns the flag when the exfiltrated token matches it.
python - <<'PY' > /tmp/lab-secrets.env
import hashlib, hmac, os
s = os.environ["LAB_USER_SECRET"].encode()
flag = "FLAG{%s}" % hmac.new(s, b"v1|reflected-xss-js-string-break-out", hashlib.sha256).hexdigest()
sess = hmac.new(s, b"admin-session|reflected-xss-js-string-break-out", hashlib.sha256).hexdigest()[:32]
print("FLAG=%s" % flag)
