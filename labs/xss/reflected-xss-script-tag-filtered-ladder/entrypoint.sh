...
flag = "FLAG{%s}" % hmac.new(s, b"v1|reflected-xss-script-tag-filtered-ladder", hashlib.sha256).hexdigest()
sess = hmac.new(s, b"admin-session|reflected-xss-script-tag-filtered-ladder", hashlib.sha256).hexdigest()[:32]
print("FLAG=%s" % flag)
