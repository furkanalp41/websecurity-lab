...
flag = "FLAG{%s}" % hmac.new(s, b"v1|bbcode-parser-img-attribute-smuggling", hashlib.sha256).hexdigest()
sess = hmac.new(s, b"admin-session|bbcode-parser-img-attribute-smuggling", hashlib.sha256).hexdigest()[:32]
print("FLAG=%s" % flag)
...
# Single worker: the topic store and admin session live in-process.
