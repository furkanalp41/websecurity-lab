...
flag = "FLAG{%s}" % hmac.new(s, b"v1|postmessage-origin-startswith-bypass", hashlib.sha256).hexdigest()
sess = hmac.new(s, b"admin-session|postmessage-origin-startswith-bypass", hashlib.sha256).hexdigest()[:32]
print("FLAG=%s" % flag)
