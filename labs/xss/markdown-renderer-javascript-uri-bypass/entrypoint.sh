...
flag = "FLAG{%s}" % hmac.new(s, b"v1|markdown-renderer-javascript-uri-bypass", hashlib.sha256).hexdigest()
sess = hmac.new(s, b"admin-session|markdown-renderer-javascript-uri-bypass", hashlib.sha256).hexdigest()[:32]
print("FLAG=%s" % flag)
