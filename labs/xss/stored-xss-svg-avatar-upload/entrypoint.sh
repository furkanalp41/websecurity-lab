...
flag = "FLAG{%s}" % hmac.new(s, b"v1|stored-xss-svg-avatar-upload", hashlib.sha256).hexdigest()
sess = hmac.new(s, b"admin-session|stored-xss-svg-avatar-upload", hashlib.sha256).hexdigest()[:32]
print("FLAG=%s" % flag)
...
# Single worker: the report queue and in-process avatar store live in-process.
