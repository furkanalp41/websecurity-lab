...
# cookie (a distinct HMAC). The admin session is the secret the CSTI must steal;
...
flag = "FLAG{%s}" % hmac.new(s, b"v1|angularjs-sandbox-escape-172", hashlib.sha256).hexdigest()
sess = hmac.new(s, b"admin-session|angularjs-sandbox-escape-172", hashlib.sha256).hexdigest()[:32]
print("FLAG=%s" % flag)
