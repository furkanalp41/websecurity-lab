...
flag = "FLAG{%s}" % hmac.new(s, b"v1|self-xss-csrf-profile-chain", hashlib.sha256).hexdigest()
sess = hmac.new(s, b"admin-session|self-xss-csrf-profile-chain", hashlib.sha256).hexdigest()[:32]
print("FLAG=%s" % flag)
