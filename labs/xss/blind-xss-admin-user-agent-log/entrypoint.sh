...
# cookie / API key (a distinct HMAC). The admin session is the secret the XSS must
# steal; /solve returns the flag when the exfiltrated key matches it.
python - <<'PY' > /tmp/lab-secrets.env
import hashlib, hmac, os
s = os.environ["LAB_USER_SECRET"].encode()
flag = "FLAG{%s}" % hmac.new(s, b"v1|blind-xss-admin-user-agent-log", hashlib.sha256).hexdigest()
sess = hmac.new(s, b"admin-session|blind-xss-admin-user-agent-log", hashlib.sha256).hexdigest()[:32]
print("FLAG=%s" % flag)
...
# Single worker: the visitor access log and admin session live in-process.
