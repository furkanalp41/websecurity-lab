...
print("FLAG{%s}" % hmac.new(s, b"v1|sqli-time-blind-mysql-sleep", hashlib.sha256).hexdigest())
PY
...
# Create + seed the analytics DB (waits for MySQL; db is already healthy via depends_on).
python /opt/app/seed.py
exec gunicorn --chdir /opt/app --bind 0.0.0.0:8080 --workers 4 --timeout 30 app:app
