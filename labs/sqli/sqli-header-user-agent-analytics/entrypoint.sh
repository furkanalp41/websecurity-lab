...
print("FLAG{%s}" % hmac.new(s, b"v1|sqli-header-user-agent-analytics", hashlib.sha256).hexdigest())
PY
...
# Create + seed the analytics store (waits for MySQL; db is already healthy via depends_on).
